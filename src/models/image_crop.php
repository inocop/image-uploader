...
class image_crop
{
    ...
    public function __construct($new_width = 250, $new_height = 250)
    {
    ...
    private function resize_and_crop(&$base_image)
    {
        ...
        if ($new_aspect_ratio < $base_aspect_ratio){
            $crop_width = $base_width * ($this->new_height / $base_height);
            $crop_height = $base_height;
        }else{
            $crop_width = $base_width;
            $crop_height = $base_height * ($this->new_width / $base_width);
        }

        if ($base_width > $base_height) {
            $crop_width = $base_height;
            $crop_height = $base_height;
            $crop_x = ($base_width - $this->new_width) / 2;
        }
        else{
            $crop_width = $base_width;
            $crop_height = $base_width;
            $crop_y = ($base_height - $this->new_height) / 2;
        }
